...
from blocks.graph import ComputationGraph, batch_normalization, get_batch_normalization_updates
...
from blocks.bricks import MLP, BatchNormalization, SpatialBatchNormalization
...
def main(name, epochs, batch_size, learning_rate, window_size, conv_sizes, num_filters, fc_dim, enc_dim, dec_dim, step, num_digits, num_classes,
    ...
    conv1_bn = SpatialBatchNormalization(input_dim=(64, 26, 26), conserve_memory=False, name='conv1_bn')
    conv2 = Convolutional(filter_size=filter_size2, num_channels=int(num_filters / 2), num_filters=int(num_filters / 2), image_size=(26, 26), name='conv2',
                          **convinits)
    conv2_bn = SpatialBatchNormalization(input_dim=(64, 24, 24), conserve_memory=False, name='conv2_bn')
    ...
    conv3_bn = SpatialBatchNormalization(input_dim=(128, 12, 12), conserve_memory=False, name='conv3_bn')
    ...
    conv4_bn = SpatialBatchNormalization(input_dim=(128, 10, 10), conserve_memory=False, name='conv4_bn')
    conv_mlp = MLP(activations=[Identity()], dims=[12800, fc_dim], name="MLP_conv", **inits)
    conv_mlp_bn = BatchNormalization(input_dim=fc_dim, conserve_memory=False, name='conv_mlp_bn')
    loc_mlp = MLP(activations=[Identity()], dims=[6, fc_dim], name="MLP_loc", **inits)
    loc_mlp_bn = BatchNormalization(input_dim=fc_dim, conserve_memory=False, name='loc_mlp_bn')
    ...
    classification_mlp1_bn = BatchNormalization(input_dim=fc_dim, conserve_memory=False, name='classification_mlp1_bn')
    classification_mlp2 = MLP(activations=[Identity()], dims=[fc_dim, fc_dim], name='MPL_class2', **inits)
    classification_mlp2_bn = BatchNormalization(input_dim=fc_dim, conserve_memory=False, name='classification_mlp2_bn')
    ...
    pop_updates = list(set(get_batch_normalization_updates(bn_cg, allow_duplicates=True)))
    ...
    update_values = [m_c1_bn, s_c1_bn, m_c2_bn, s_c2_bn, m_c3_bn, s_c3_bn, m_c4_bn, s_c4_bn, m_c_bn, s_c_bn, m_l_bn, s_l_bn, m_cl1_bn, s_cl1_bn, m_cl2_bn,
                     s_cl2_bn]

    pop_updates.extend([(p, m.mean(axis=0)) for p, m in zip(update_params, update_values)])
    ...
    if oldmodel is not None:
        ...
        with open(oldmodel, "rb") as f:
            ...
            main_loop.model.get_top_bricks()[0].conv_init.layers[4].population_mean.set_value(
                oldmodel.get_top_bricks()[0].conv_init.layers[4].population_mean.get_value())
            main_loop.model.get_top_bricks()[0].conv_init.layers[4].population_stdev.set_value(
                oldmodel.get_top_bricks()[0].conv_init.layers[4].population_stdev.get_value())

            main_loop.model.get_top_bricks()[0].conv_init.layers[7].population_mean.set_value(
                oldmodel.get_top_bricks()[0].conv_init.layers[7].population_mean.get_value())
            main_loop.model.get_top_bricks()[0].conv_init.layers[7].population_stdev.set_value(
                oldmodel.get_top_bricks()[0].conv_init.layers[7].population_stdev.get_value())
        del oldmodel
    main_loop.run()
